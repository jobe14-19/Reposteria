...
public class PedidoModalController {
    ...
    private static final String SQL_OBTENER_PRECIOS_PRODUCTO =
            "SELECT precio_base, costo_diseño FROM productos WHERE nombre = ?";

    private static final String SQL_ACTUALIZAR_PEDIDO =
            "UPDATE pedidos SET id_cliente = ?, fecha_entrega = ?, id_producto = ?, libras = ?, diseno = ?, total = ?, adelanto = ?, observaciones = ?, fecha_modificacion = GETDATE() WHERE id_pedido = ?";
    ...
    private static final String TITULO_EDITAR = "✏️ Editar Pedido";
            ...
            if (stmt.executeUpdate() > 0) {
                try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                        ...
                        mostrarMensaje("Pedido Creado", "El pedido ha sido creado correctamente.");
                        cerrarModal();
                    }
                }
            } else {
            ...
            try (Connection conn = dbConnection.getConnection();
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        double costoDiseño = rs.getDouble("costo_diseño");
