    ...
    private void cargarDatosUsuario() {
        try (Connection conn = dbConnection.getConnection()) {
            ...
            if (datosUsuario != null) {
                ...
                mostrarError("Error", "No se pudieron cargar los datos del usuario");
            }

        } catch (SQLException e) {
            LOGGER.log(Level.SEVERE, "Error al cargar datos del usuario: {0}", e.getMessage());
            mostrarError("Error de Base de Datos", "No se pudieron cargar los datos del usuario: " + e.getMessage());
        }
    }
