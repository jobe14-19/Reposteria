...
public class MiPerfilController {
 ...
 private static final String SQL_UPDATE_CLIENTE_PASSWORD = "UPDATE clientes SET contrasena = ? WHERE id_cliente = ?";
  ...
  private static final String SQL_UPDATE_USUARIO_PASSWORD = "UPDATE usuarios SET contrasena = ? WHERE usuario = ?";
 ...
 return false;
  ...
  if (PERFIL_CLIENTE.equals(perfil)) {
  try (PreparedStatement stmt = conn.prepareStatement(SQL_UPDATE_CLIENTE_PASSWORD)) {
  stmt.setString(1, nuevaContrasena);
  stmt.setInt(2, idUsuario);
  return stmt.executeUpdate() > 0;
  }
  } else {
  try (PreparedStatement stmt = conn.prepareStatement(SQL_UPDATE_EMPLEADO_PASSWORD)) {
  stmt.setString(1, nuevaContrasena);
  stmt.setInt(2, idUsuario);
  stmt.executeUpdate();
  }
  try (PreparedStatement stmt = conn.prepareStatement(SQL_UPDATE_USUARIO_PASSWORD)) {
  stmt.setString(1, nuevaContrasena);
