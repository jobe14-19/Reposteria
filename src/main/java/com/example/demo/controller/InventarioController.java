...
public class InventarioController {
 ...
 if (!sessionManager.tienePermiso(Permiso.INVENTARIO_LEER)) {
 mostrarError("Acceso Denegado", "No tienes permiso para acceder al inventario.");
 return;
 }
 ...
 boolean puedeEditar = sessionManager.tienePermiso(Permiso.INVENTARIO_ACTUALIZAR);
 boolean puedeEliminar = sessionManager.tienePermiso(Permiso.INVENTARIO_ELIMINAR);
...
private final Button eliminarButton = new Button("Eliminar");
 ...
 hbox.getChildren().add(eliminarButton);
 }
 ...
 abrirVista("HistorialCompras.fxml", "Historial de Compras", 900, 600);
 }

 @FXML
 private void actualizarStock(ActionEvent event) {
 abrirVista("StockMovimientos.fxml", "Movimientos de Stock", 1000, 600);
 }
 ...
 net.sf.jasperreports.engine.JasperReport report = rs.compileReport("/reportes/Reporte_Inventario.jrxml");
 ...
 alert.setTitle("Error en Reporte de Inventario");
 alert.setHeaderText("No se pudo generar el reporte");
 alert.setContentText(e.getMessage() + "\n\nVerifique que la tabla 'ingredientes' exista y tenga datos.");
 alert.showAndWait();
 }
 }
 ...
 private void abrirVista(String fxml, String titulo, int ancho, int alto) {
 try {
 FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/demo/" + fxml));
 javafx.scene.Parent root = loader.load();
 Stage stage = new Stage();
 stage.setScene(new Scene(root, ancho, alto));
 stage.setTitle(" Repostería Rosato - " + titulo);
 stage.initModality(javafx.stage.Modality.APPLICATION_MODAL);
 stage.showAndWait();
 // Refresh inventory after viewing
 cargarInventario();
 cargarAlertas();
 } catch (Exception e) {
 LOGGER.log(Level.SEVERE, "Error al abrir {0}: {1}", new Object[]{fxml, e.getMessage()});
