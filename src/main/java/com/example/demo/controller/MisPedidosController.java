...
private final Button seguimientoButton = new Button("Seguimiento");
private final Button facturaButton = new Button("Factura");
private final Button detalleButton = new Button("Detalle");
 private final HBox hbox = new HBox(5);
 ...
 try {
 var loader = new FXMLLoader(getClass().getResource("/com/example/demo/PedidoModal.fxml"));
 var root = loader.<Parent>load();

 var controller = loader.<PedidoModalController>getController();
 controller.setPedido(null);
 ...
 mostrarMensaje("Seguimiento de Pedido", mensaje);
  ...
  LOGGER.log(Level.SEVERE, "Error al generar factura", e);
  mostrarError("Error", "No se pudo generar la factura: " + e.getMessage());
  }
  }
