...
public class CompraModalController {
 ...
 @FXML private Label subtotalItemLabel;
 @FXML private TextField subtotalField;
 @FXML private TextField totalDescuentoField;
 @FXML private TextField impuestosField;
 @FXML private TextField alquilerField;
 @FXML private TextField garantiaField;
 @FXML private TextField abonoField;
 ...
 totalDescuentoField.textProperty().addListener((obs, oldVal, newVal) -> recalcularTotalGeneral());
 impuestosField.textProperty().addListener((obs, oldVal, newVal) -> recalcularTotalGeneral());
 alquilerField.textProperty().addListener((obs, oldVal, newVal) -> recalcularTotalGeneral());
 garantiaField.textProperty().addListener((obs, oldVal, newVal) -> recalcularTotalGeneral());
 abonoField.textProperty().addListener((obs, oldVal, newVal) -> recalcularSaldo());
 }
 ...
 recalcularTotalGeneral();
 double total = Double.parseDouble(obtenerTextoODefault(totalField, "0"));
 ...
 for (ProductoCompra producto : productosList) {
 int idIngrediente = stockDAO.buscarIdIngredientePorNombre(producto.getNombre());
 if (idIngrediente > 0) {
 stockDAO.registrarEntrada(idIngrediente, producto.getNombre(), producto.getCantidad(),
 "Compra de insumos", "COMPRA", 0, usuario);
 }
 }
 }
 ...
 double subtotal = calcularTotalCompra();
 double descuentoAdicional = Double.parseDouble(obtenerTextoODefault(totalDescuentoField, "0"));
 double impuestos = Double.parseDouble(obtenerTextoODefault(impuestosField, "0"));
 double alquiler = Double.parseDouble(obtenerTextoODefault(alquilerField, "0"));
 double garantia = Double.parseDouble(obtenerTextoODefault(garantiaField, "0"));

 double total = subtotal - descuentoAdicional + impuestos + alquiler + garantia;
 subtotalField.setText(String.format("%.2f", subtotal));
 totalField.setText(String.format("%.2f", Math.max(0, total)));
 ...
 double total = Double.parseDouble(obtenerTextoODefault(totalField, "0"));
 double abono = Double.parseDouble(obtenerTextoODefault(abonoField, "0"));
 saldoField.setText(String.format("%.2f", Math.max(0, total - abono)));
 }
