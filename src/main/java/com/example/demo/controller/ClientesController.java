...
private static final String SQL_ELIMINAR_CLIENTE = "UPDATE clientes SET estado = 'Inactivo' WHERE id_cliente = ?";
 ...
 if (!sessionManager.tienePermiso(Permiso.CLIENTES_LEER)) {
 mostrarError("Acceso Denegado", "No tienes permiso para acceder a la gestión de clientes.");
 return;
 }
 ...
 boolean puedeEliminar = sessionManager.tienePermiso(Permiso.CLIENTES_ELIMINAR);

 accionesColumn.setCellFactory(param -> new TableCell<Cliente, Void>() {
private final Button editarButton = crearBoton("Editar", BTN_EDITAR_STYLE);
private final Button eliminarButton = crearBoton("Eliminar", BTN_ELIMINAR_STYLE);
private final Button verPedidosButton = crearBoton("Pedidos", BTN_PEDIDOS_STYLE);
 ...
 hbox.getChildren().setAll(editarButton, verPedidosButton);
 }
 setGraphic(hbox);
 }
 }
 ...
 if (!sessionManager.tienePermiso(Permiso.CLIENTES_CREAR)) {
 mostrarError("Acceso Denegado", "No tienes permiso para crear nuevos clientes.");
 return;
 }
