...
public class RegistroClienteController {
 ...
 OptionalInt idGenerado = dbConnection.insertarCliente(nuevoCliente);
  ...
  int id = idGenerado.getAsInt();
  SessionManager.getInstance().iniciarSesion(id, nombre, "CLIENTE");

  Alert alert = new Alert(Alert.AlertType.INFORMATION);
  alert.setTitle("Registro Exitoso");
  alert.setHeaderText("Bienvenido, " + nombre + "!");
  alert.setContentText("Tu cuenta ha sido creada. Seras redirigido a tu panel.");
  alert.showAndWait();
 ...
 mostrarError("Error al registrar el usuario: " + e.getMessage());
  ...
  try {
  FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/demo/DashboardCliente.fxml"));
  Parent root = loader.load();
  Stage stage = (Stage) registerButton.getScene().getWindow();
  Scene scene = new Scene(root, 1000, 700);
  stage.setScene(scene);
  stage.setTitle("Reposteria Rosato - Panel de Cliente");
  stage.show();
  } catch (IOException e) {
  LOGGER.log(Level.SEVERE, "Error al ir al dashboard: {0}", e.getMessage());
