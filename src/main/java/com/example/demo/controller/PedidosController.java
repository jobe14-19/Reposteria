...
public class PedidosController {
 ...
 @FXML private TableColumn<Pedido, Double> adelantoColumn;
  ...
  @FXML private TableColumn<Pedido, Void> accionesColumn;
 ...
 if (!sessionManager.tienePermiso(Permiso.PEDIDOS_LEER)) {
 mostrarError("Acceso Denegado", "No tienes permiso para acceder a la gestión de pedidos.");
 return;
 }
 ...
 clienteFilter.getSelectionModel().selectFirst();
  ...
  private static final String BTN_FACTURA_STYLE = "-fx-background-color: #28A745; -fx-text-fill: white; -fx-font-size: 10px; -fx-font-weight: bold; -fx-padding: 5 10 5 10; -fx-background-radius: 4; -fx-cursor: hand;";
  ...
  { facturaButton.setStyle(BTN_FACTURA_STYLE);
  ...
  setGraphic(empty ? null : facturaButton);
  }
  });
 ...
 mostrarError("Error", "No se pudo abrir la ventana de pedido: " + e.getMessage());
  ...
  params.put("ID_FACTURA", pedido.getId());
  ...
  mostrarError("Error al generar factura", "No se pudo generar la factura: " + e.getMessage());
  }
  }
