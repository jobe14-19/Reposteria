...
private static final String SQL_ELIMINAR_EMPLEADO = "UPDATE empleados SET estado = 'Inactivo' WHERE id_empleado = ?";
 ...
 private static final String AREA_VENTAS = "Ventas";
 private static final String AREA_ATENCION_CLIENTE = "Atención al Cliente";
 private static final String AREA_LIMPIEZA = "Limpieza";
 private static final String AREA_ADMINISTRACION = "Administración";
 ...
 if (!sessionManager.tienePermiso(Permiso.PERSONAL_LEER)) {
 mostrarError("Acceso Denegado", "No tienes permiso para acceder a la gestión de personal.");
 return;
 }
 ...
 areaFilter.getItems().addAll(AREA_TODOS, AREA_PRODUCCION, AREA_DECORACION, AREA_DELIVERY, AREA_VENTAS, AREA_ATENCION_CLIENTE, AREA_LIMPIEZA, AREA_ADMINISTRACION);
 ...
 boolean puedeEditar = sessionManager.tienePermiso(Permiso.PERSONAL_ACTUALIZAR);
...
private final Button eliminarButton = new Button("Eliminar");
 ...
 "ELSE 'Pendiente' END as capacitacion " + 
"FROM empleados e " + 
"WHERE e.estado = 'Activo' AND (e.nombre LIKE ? OR e.cedula LIKE ? OR e.telefono LIKE ?) " + 
 "ORDER BY e.nombre";
 ...
 if (!sessionManager.tienePermiso(Permiso.PERSONAL_CREAR)) {
 mostrarError("Acceso Denegado", "No tienes permiso para contratar nuevos empleados.");
 return;
 }
 ...
 stage.setTitle(empleado == null ? " Contratar Empleado" : " Editar Empleado");
